...
import categoryDescriptions from "./constants/desc.js";
...
function RulesModal({ isOpen, rulesImage, rulesText, onClose }) {
  ...
  const items = [];
  if (rulesImage) {
    if (Array.isArray(rulesImage))
      rulesImage.forEach((img) => items.push({ img }));
    else items.push({ img: rulesImage });
  }
  if (rulesText) {
    if (Array.isArray(rulesText))
      rulesText.forEach((t) => items.push({ text: t }));
      ...
      >
        ...
        <div className="flex flex-col gap-4 items-center">
          <div className="relative w-full flex items-center justify-center">
            ...
            <div
              ref={carouselRef}
              className="rounded-lg overflow-hidden bg-black/30 flex items-center justify-center"
            >
              {total > 0 ? (
                items[carouselIndex].img ? (
                  <img
                    src={items[carouselIndex].img}
                    alt="Rules"
                    className="max-w-full max-h-[80vh] object-contain"
                  />
                ) : (
                  <div className="p-6 text-white/80 max-w-[80vw] whitespace-pre-wrap">
                    {items[carouselIndex].text}
                  </div>
                )
              ) : (
...
function PosterModal({ isOpen, posterImage, onClose }) {
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="relative w-auto max-w-[90vw] mx-4 bg-white/5 rounded-2xl p-4 border border-white/10 max-h-[90vh] overflow-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
  ...
  useEffect(() => {
    if (isOpen && modalRef.current) {
      gsap.fromTo(
        modalRef.current,
        { opacity: 0, y: 30 },
        { opacity: 1, y: 0, duration: 0.33, ease: "power3.out" },
      );
    }
  }, [isOpen]);
...
function EventItem({ event, onRegister, index, category }) {
  ...
  return (
    <>
      <div
        ...
        style={{
          backgroundImage: event.image
            ? `linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url(${event.image})`
            : undefined,
      ...
      >
        {!event.image && (
          <div className="absolute inset-0 bg-linear-to-r from-[#071026] to-[#071018]" />
        )}
        ...
        <div className="relative p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3
              className="text-2xl md:text-4xl font-extrabold uppercase tracking-tight text-white mb-2"
              style={{ color: isHoveblue ? "#7EE7D5" : "#ffffff" }}
            >
              {event.title}
            </h3>
            ...
            <div className="md:hidden flex items-center justify-between gap-3">
              ...
              {!hideButtons.includes(event.title) && (
                <a
                  ...
                  rel={
                    event.registrationLink ? "noopener noreferrer" : undefined
                  }
                  onClick={(e) => {
...
export default function EventDescription() {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-[#02040a] via-[#031028] to-black p-8 md:p-12 pt-36 md:pt-44 relative z-20">
      ...
      <div
        ref={headerRef}
        className="fixed top-0 left-0 w-full z-30 border-b border-white/20 bg-[#02040a]"
      >
